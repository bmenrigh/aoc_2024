...
def solve_maze(grid, start, end, max_cheat, min_imp, best, c_loc, best_mem, savemem):
    ...
    def s_rec(cx, cy, px, py, s, mem, cheat_sx, cheat_sy, cheat_ex, cheat_ey, cheating, c_path, cheated, c_count, c_mem):
        ...
        if grid[cx][cy] == "#" and not cheating:
                return -1
        ...
        if not best_mem is None:
            if (cx, cy) in best_mem:
                if s < best_mem[(cx, cy)]:
                    if savemem and not cheating and not cheated:
                        best_mem[(cx, cy)] = s

                if cheated:
                    if s > best_mem[(cx, cy)] - min_imp:
                        return -1
        ...
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if not cheating and not cheated:
                ns = s_rec(nx, ny, cx, cy, s + 1, mem, cx, cy, cheat_ex, cheat_ey, True, {}, cheated, c_count, c_mem)
...
best_mem = {}
best = solve_maze(grid, start, end, 0, 0, 0, {}, best_mem, True)
print(f"best to beat {best}")
c_loc = {}
for i in range(2, 21):
    nb = solve_maze(grid, start, end, i, 100, best, c_loc, best_mem, False)
    ...
    print(f"cheat len {i} done with {beat_count} unique cheats so far")
